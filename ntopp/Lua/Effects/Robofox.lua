...
rawset(_G, "PT_NotScaredList", function(types) 
//fk you green snapper, you made me look bad!
if (types == MT_GSNAPPER)
or (types == MT_SNAPPER_LEG)
or (types == MT_SNAPPER_HEAD)
//deton kinda breaks when being scared
or (types == MT_DETON)
return true
else
return false
...
addHook("MobjSpawn", function(mobj)//var
if (mobj.flags & MF_ENEMY) and not PT_NotScaredList(mobj.type) then
mobj.scared = 0
mobj.madcooldown = 0
end
end, MT_NULL)

addHook("MobjThinker", function(mobj)//nil.
if (mobj.flags & MF_ENEMY) and not PT_NotScaredList(mobj.type) then
mobj.scared = $ or 0
mobj.madcooldown = $ or 0
end
end, MT_NULL)

addHook("MobjThinker", function(mobj)//dennis
if not mobj or not mobj.valid then return end
if not(mobj.flags & MF_ENEMY) or PT_NotScaredList(mobj.type) or (mobj.health == 0) then return end
	P_LookForPlayers(mobj, 200*mobj.scale, true, true)    
		if mobj.scared > 0 then
			mobj.scared = $ - 1
		end
		 if mobj.madcooldown > 0 then
			mobj.madcooldown = $ - 1
		end
        if mobj.tracer
		and mobj.tracer.player then
			if isPTSkin and ntopp_v2
		        and isPTSkin(mobj.tracer.player.mo.skin)
		        and mobj.tracer.player.fsm then
		        if (mobj.tracer.player.fsm.state == ntopp_v2.enums.MACH3)
					if mobj.scared < 1 then
						...
						elseif (mobj.tracer.player.fsm.state == ntopp_v2.enums.TAUNT) and mobj.madcooldown < 1 then
						if (mobjinfo[mobj.type].missilestate != S_NULL)
						mobj.state = mobjinfo[mobj.type].missilestate
					    mobj.madcooldown = 35
						elseif (mobjinfo[mobj.type].meleestate != S_NULL)
						mobj.state = mobjinfo[mobj.type].meleestate
					    mobj.madcooldown = 35
						elseif (mobjinfo[mobj.type].seestate != S_NULL)
						mobj.state = mobjinfo[mobj.type].seestate
					    mobj.madcooldown = 35
						else
						mobj.state = mobjinfo[mobj.type].spawnstate
					    mobj.madcooldown = 35
